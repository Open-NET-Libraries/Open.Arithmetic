...
public static class Statistical
{
	...
	public static double Variance(this ReadOnlySpan<double> source, bool sample = false)
	{
	...
	/// <inheritdoc cref="Variance(ReadOnlySpan{double}, bool)"/>
	public static double Variance(this Span<double> source, bool sample = false)
	{
		// Manual calculation to be sure LINQ isn't screwing up our double precision. :(
	...
	public static double ProductsSum(this ReadOnlySpan<double> source, ReadOnlySpan<double> target)
	{
	...
	public static double ProductsSum(this Span<double> source, ReadOnlySpan<double> target)
	{
		var len = source.Length;
		if (len != target.Length) throw new ArgumentException("Products: source and target enumerations have different counts.");
		double sum = 0;
	...
	public static IEnumerable<double> Products(this IEnumerable<double> source, IEnumerable<double> target)
	{
		...
		return ProductsCore(source, target);

		static IEnumerable<double> ProductsCore(IEnumerable<double> source, IEnumerable<double> target)
		{
	...
	static (double, double, int) DualSums(ReadOnlySpan<double> source, ReadOnlySpan<double> target, bool sample)
	{
	...
	/// <param name="sample">If true, will divide by (n-1) (population).  If false, will divide by (n) (sample).</param>
	/// <returns>The variance of a set of numbers.</returns>
	public static double Covariance(this ReadOnlySpan<double> source, ReadOnlySpan<double> target, bool sample = false)
	{
		var (sumA, sumB, n) = DualSums(source, target, sample);

		var prod = ProductsSum(source, target);
	...
	/// <inheritdoc cref="Correlation(double, double, double)"/>
	public static double Correlation(double covariance, ReadOnlySpan<double> source, ReadOnlySpan<double> target)
		=> Correlation(covariance, Variance(source), Variance(target));

	public static double Correlation(this ReadOnlySpan<double> source, ReadOnlySpan<double> target)
		=> Correlation(Covariance(source, target), source, target);

	public static double Correlation(this Span<double> source, ReadOnlySpan<double> target)
		=> Correlation((ReadOnlySpan<double>)source, target);
